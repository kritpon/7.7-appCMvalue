Public Class frmBegin
    ...
    Sub showData()
        ...
        txtSQL = "Select TranDataH_E.Trh_Date,TranDataH_E.Trh_KeyType,TranDataH_E.Trh_No,"
        ...
        txtSQL = txtSQL & "On TranDataH_E.trh_No=BOMmastH.BOM_No "
        ...
        txtSQL = txtSQL & "or BOMmastD.BOM_RM_Code='08001' "
        txtSQL = txtSQL & "or BOMmastD.BOM_RM_Code='08002' "
        txtSQL = txtSQL & "or BOMmastD.BOM_RM_Code='08003' "
        txtSQL = txtSQL & "or BOMmastD.BOM_RM_Code='08004' "
        txtSQL = txtSQL & "or BOMmastD.BOM_RM_Code='08005' "
        ...
        txtSQL = txtSQL & "And TranDataH_e.Trh_Date='" & strDateNow1 & "' "
        ...
        txtSQL = txtSQL & "or TranDataH_e.Trh_Date='" & strDateNow2 & "' "
        txtSQL = txtSQL & "Group by TranDataH_E.Trh_Date,TranDataH_E.Trh_KeyType,TranDataH_E.Trh_No "
        ...
        With subds.Tables("Master")
            ...
            For i = 0 To subds.Tables("Master").Rows.Count - 1
                qtyNo = .Rows(i).Item("sum08005")
                lbQty.Text = qtyNo
                strDate1 = Format(DateAdd(DateInterval.Year, -543, (subds.Tables("Master").Rows(i).Item("Trh_Date"))), "yyyy/MM/dd")
                ...
                If strDateNow1 = strDate1 Then
                    If .Rows(i).Item("Trh_KeyType") = "A" Then

                        If .Rows(i).Item("sum07001") > 0 Then
                            lb10sA_1.Text = Format((.Rows(i).Item("sum07001") * qtyNo) + CDbl(lb10sA_1.Text), "#,##0")
                        ElseIf .Rows(i).Item("sum07002") > 0 Then
                            lb22sA_1.Text = Format(.Rows(i).Item("sum07002") * qtyNo + CDbl(lb22sA_1.Text), "#,##0")
                        ElseIf .Rows(i).Item("sum07003") > 0 Then
                            lb60sA_1.Text = Format(.Rows(i).Item("sum07003") * qtyNo + CDbl(lb60sA_1.Text), "#,##0")
                        End If

                    Else

                        If .Rows(i).Item("sum07001") > 0 Then
                            lb10sB_1.Text = Format(.Rows(i).Item("sum07001") * qtyNo + CDbl(lb10sB_1.Text), "#,##0")
                        ElseIf .Rows(i).Item("sum07002") > 0 Then
                            lb22sB_1.Text = Format(.Rows(i).Item("sum07002") * qtyNo + CDbl(lb22sB_1.Text), "#,##0")
                        ElseIf .Rows(i).Item("sum07003") > 0 Then
                            lb60sB_1.Text = Format(.Rows(i).Item("sum07003") * qtyNo + CDbl(lb60sB_1.Text), "#,##0")
                        End If
                ...
                ElseIf strDateNow2 = strDate1 Then ' subds.Tables("Master").Rows(i).Item("Trh_Date") Then
                    ...
                    If .Rows(i).Item("Trh_KeyType") = "A" Then
                        ...
                        If .Rows(i).Item("sum07001") > 0 Then
                            lb10sA_2.Text = Format((.Rows(i).Item("sum07001") * qtyNo) + CDbl(lb10sA_1.Text), "#,##0")
                        ElseIf .Rows(i).Item("sum07002") > 0 Then
                            lb22sA_2.Text = Format(.Rows(i).Item("sum07002") * qtyNo + CDbl(lb22sA_1.Text), "#,##0")
                        ElseIf .Rows(i).Item("sum07003") > 0 Then
                            lb60sA_2.Text = Format(.Rows(i).Item("sum07003") * qtyNo + CDbl(lb60sA_1.Text), "#,##0")
                        End If

                    Else

                        If .Rows(i).Item("sum07001") > 0 Then
                            lb10sB_2.Text = Format(.Rows(i).Item("sum07001") * qtyNo + CDbl(lb10sB_1.Text), "#,##0")
                        ElseIf .Rows(i).Item("sum07002") > 0 Then
                            lb22sB_2.Text = Format(.Rows(i).Item("sum07002") * qtyNo + CDbl(lb22sB_1.Text), "#,##0")
                        ElseIf .Rows(i).Item("sum07003") > 0 Then
                            lb60sB_1.Text = Format(.Rows(i).Item("sum07003") * qtyNo + CDbl(lb60sB_1.Text), "#,##0")
                        End If
    ...
    Sub fmtListView()
        ...
        With lsvData
            ...
            .Columns.Add("ชื่อชุดงาน", 530, HorizontalAlignment.Left) '0
            .Columns.Add("Scale", 100, HorizontalAlignment.Center) '0
            .Columns.Add("เวลาเริ่ม", 400, HorizontalAlignment.Right) '0
            .Columns.Add("รอ", 350, HorizontalAlignment.Right) '0
    ...
    Sub showDataList()
        ...
        txtSQL = txtSQL & "Order by CM_Mix_DateTime desc"
        ...
        For i = 0 To subds.Tables("Master").Rows.Count - 1
            With subds.Tables("Master").Rows(i)
                ...
                anydata = New String() {(i + 1).ToString("#,##0"), strDocNo, strStkName, strScale, strTime, strConVTime, dblTime.ToString("#,##0")}
